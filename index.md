...
Lazy Evaluation, also called call-by-need, delays the evaluation of an expression until the value is needed [7nn]. It is a useful technique which allows the delaying of the evaluation of code until the result is required.\
...
# Golang
Golang, or short Go, is  an open source project that is being developend by a team at Google and the open source community [10n]. Robert Griesemer, Rob Pike and Ken Thompson thought about a new language on September 21, 2007. This language became Go and a open source project on November 10,2009 [11n]. The first release was in March 2012 with the version '1' and in september 2019 was the latest release with version '1.13' [10n]. Go tries to reduce clutter and complexity by removing ancestors and header files [11n].\
...
### Closures in Go
Go directly implements closures [12nn]:
```
...
Lazy evaluation is therefore not directly implemented in Go in general, but it is possible to implement by using clojures and the sync package [13nn]:
```
...
```
The withers create a new state, the getters return data and the setters mutate state. The functions receive no pointer to make sure that the structure is passed by  and returned by value [14n].
...
# Objective C
Objective C is the main programming language for writing software for OS X and iOS. Objective C is a superset of the C programming language and inherits many characeristic of C while also adding object-oriented capabilities and a dynamic runtime [15n]. There are currently two versions of Objective C: 'modern' and 'legacy'. The modern version is Objective C 2.0 with the programming interface "Objective-C Runtime Reference" while the legacy version is described in the "Objective-C 1 Runtime Reference" [15n].\
...
The use of 'blocks' allows to create distinct segements of code to be passed to methods or functions as if they were values [16n].
...
```
The 'void(^)(double, double))' is the block that gets passed as a parameter in this case [16n].
...
```
The first and second example seem to be equivalent in terms of the return value. They may not be equivalent in terms of performance or side effect however. If 'b' is costly to evaluate or has side effects, it does not get called in the latter example [17n].\
...
```
The code in the blocks is not evaluated until the blocks are called [17n].\
An implementation for lazy evaluation in Objective C can be found in [17n]. It uses a technique called isa-swizzling, which means changing the isa pointer on an live object to point to a subclass of the original class. The implemenation is rather large and would go beyond the scope for this paper. It is important to know that lazy evaluation is possible for Objective C.
